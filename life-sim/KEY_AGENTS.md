# Schlüsselagenten & Berater
## Stand: 1. Juni 2026

> Agenten sind Ethans wichtigste externe Kontakte.
> Gute Beziehungen = schnellere Deals, bessere Konditionen.
> Template unten für neue Agenten.

---

## TEMPLATE

```
### [AGENTENNAME] — [AGENTUR]
| Feld | Info |
|------|------|
| Name | |
| Agentur | |
| Nationalität | |
| Verhältnis zu Ethan | |
| Relevante Liverpool-Spieler | |
| Relevante Transferziele | |
| Stil / Verhandlungsprofil | |
| Status | 🟢 Guter Kontakt / 🟡 Neutral / 🔴 Schwierig |
```

---

## BEKANNTE AGENTEN

### Jorge Mendes — Gestifute
| Feld | Info |
|------|------|
| Name | Jorge Mendes |
| Agentur | Gestifute |
| Nationalität | 🇵🇹 Portugiesisch |
| Verhältnis zu Ethan | Noch kein persönlicher Kontakt |
| Relevante Liverpool-Spieler | Diverse Premier League Spieler |
| Relevante Transferziele | Diverse — einer der mächtigsten Agenten der Welt |
| Stil | Hartnäckig, Deal-orientiert, bevorzugt große Klubs. Direkt und respektiert Gegenseitigkeit. |
| Status | 🟡 Erstkontakt hergestellt 06.06. — Adeyemi-Gespräch geführt, sauber beendet. Tür offen. |

---

### Rafaela Pimenta — Pimenta Sports
| Feld | Info |
|------|------|
| Name | Rafaela Pimenta |
| Agentur | Pimenta Sports (ehem. Raiola-Agentur) |
| Nationalität | 🇧🇷 Brasilianisch |
| Verhältnis zu Ethan | Noch kein persönlicher Kontakt |
| Relevante Liverpool-Spieler | Ryan Gravenberch, Alexander Isak |
| Relevante Transferziele | Viele Top-Mittelfeldspieler europaweit |
| Stil | Professionell, direkt, sehr gut vernetzt in den Niederlanden |
| Status | 🟢 Sofort wichtig — vertritt Gravenberch + Isak |

---

### Ramy Abbas Issa
| Feld | Info |
|------|------|
| Name | Ramy Abbas Issa |
| Agentur | Unabhängig |
| Nationalität | 🇸🇾 Syrisch-Britisch |
| Verhältnis zu Ethan | Noch kein persönlicher Kontakt |
| Relevante Liverpool-Spieler | Mohamed Salah *(verlässt Klub)* |
| Relevante Transferziele | Diverse arabische / internationale Spieler |
| Stil | Loyal zu Spielern, manchmal provokativ in Verhandlungen |
| Status | 🟡 Relevant für Salah-Abwicklung |

---

### BASE Soccer (Virgil van Dijk)
| Feld | Info |
|------|------|
| Name | BASE Soccer / Sasha Empson |
| Agentur | BASE Soccer |
| Nationalität | 🇬🇧 Britisch |
| Verhältnis zu Ethan | Noch kein persönlicher Kontakt |
| Relevante Liverpool-Spieler | Virgil van Dijk |
| Relevante Transferziele | — |
| Stil | Ruhig, professionell |
| Status | 🔴 Kritisch — Van Dijk-Vertrag läuft 2027. Gespräch nötig |

---

### CAA Stellar (Wilfried Gnonto)
| Feld | Info |
|------|------|
| Name | CAA Stellar |
| Agentur | CAA Stellar (Teil von Creative Artists Agency) |
| Hauptsitz | London, UK |
| Nationalität | 🇬🇧 Britisch (global) |
| Verhältnis zu Ethan | Noch kein persönlicher Kontakt |
| Relevante Liverpool-Spieler | — |
| Relevante Transferziele | **Wilfried Gnonto** (Leeds United) |
| Stil | Professionelle Großagentur — viele PL-Spieler, strukturierte Verhandlungen |
| Status | 🟡 Kontakt anstehend — informelle Sondierung geplant |

**Kontaktweg:** Standard-Industrie-Kanal über Liverpool Sekretariat oder direkt über PL-Netzwerk. CAA Stellar haben ein Londoner Büro — leicht erreichbar.

---

### MM-Management (Nicolò Cambiaghi)
| Feld | Info |
|------|------|
| Name | MM-Management |
| Agentur | MM-Management |
| Hauptsitz | Mailand, Italien |
| Nationalität | 🇮🇹 Italienisch |
| Verhältnis zu Ethan | Erstkontakt initiiert (06. Juni 2026) |
| Relevante Liverpool-Spieler | — |
| Relevante Transferziele | **Nicolò Cambiaghi** (FC Bologna) |
| Stil | Italienische Spielerberatung — mittelgroße Agentur, erfahren im Serie A Markt |
| Status | 🟡 Erstkontakt hergestellt — Gespräch steht aus |

**Kontaktweg:** Direktanruf 06. Juni 2026 eingeleitet. Agent war überrascht aber professionell. Gespräch wurde noch nicht geführt — Ethan hat es vorerst als Sondierung markiert.

---

## AGENTEN RELEVANTER TRANSFERZIELE

| Spieler | Agent (bekannt/geschätzt) | Agentur | Notiz |
|---------|--------------------------|---------|-------|
| **Nico Schlotterbeck** | **Björn Etzel** | Unabhängig | ✅ Gespräch 06.06. — Release €58M bestätigt, Lohnlücke £50k/Woche, Wiederkontakt Juli |
| Bradley Barcola | — | — | Zu recherchieren |
| Yan Diomande | — | — | Zu recherchieren |
| Marco Senesi | — | — | Free Transfer — direkt ansprechen |
| Jérémy Jacquet | — | — | ✅ Bereits verpflichtet — kein Agent-Kontakt nötig |
| **Gonçalo Inácio** | **Miguel Pinho** | Pinho Sports | ✅ Gespräch 06.06. — €45-52M Fee, £110-120k Lohn. Treffen nach WM vereinbart. |
| **Nicolò Cambiaghi** | MM-Management | MM-Management, Mailand | ✅ Identifiziert — Erstkontakt 06.06. |
| **Wilfried Gnonto** | CAA Stellar | CAA Stellar, London | ✅ Identifiziert — Kontakt geplant |

---

## ZUKÜNFTIGE ERGÄNZUNGEN

*Wird ergänzt wenn:*
- Transfergespräche beginnen
- Neue Agenten-Kontakte entstehen
- Verhandlungen zu spezifischen Spielern starten
